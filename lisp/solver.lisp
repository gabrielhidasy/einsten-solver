(make-package 'harvey_dent2)
(defpackage :harvey_dent2
  (:use :common-lisp)
  (:export "solver"
   ))
(in-package harvey_dent2)
...
(defun presolve (constraints)
  "This function normalize the rules, making all - +, > <, ..."
  (let ((adjusted-constraints '()))
    (dolist (constraint constraints)
      ;;Transforma regras do tipo (= coisa1 ([+/-] coisa2 1)) na mais tradicional
      ;;(= ([+/-] coisa 1) coisa)
      (when (and (equal (car constraint) '=)
		 (symbolp (cadr constraint))
		 (listp (caddr constraint)))
	...
	((and (equal (car constraint) '=)
	      (numberp (cadr constraint))
	      (symbolp (caddr constraint)))
	...
	;;(= (+ coisa2 n) coisa1)
	((and (equal (car constraint) '=)
	      (listp (cadr constraint))
	      (equal (caadr constraint) '-))
	...
	((and (equal (car constraint) '=)
	      (listp (cadr constraint))
	      (equal (caadr constraint) 'ABS)
	      (equal (car (cadadr constraint)) '+))
...
thing n) rules, or thing-house rules, and do some preprocessing  with <, +"
  (let ((adjusted-constraints '()))
    (dolist (constraint constraints)
      (cond
	...
	;;Se A < B, A não pode estar na ultima casa e vice-versa
	((and (equal (car constraint) '<))
	...
	(push constraint adjusted-constraints))
	;;Se (A + n) = B, A não pode estar nas ultimas n casas e vice-versa
	((and (equal (car constraint) '=)
	      (listp (cadr constraint))
	      (equal '+ (caadr constraint)))
...
(defun fix-field (&optional y)
  "when an item can only be in houses a, house a will have only this item"
  (proclaim '(fixnum x y z counter-1 pos-1 counter-2 pos-2))
  (when (eql y nil)
    (dotimes (k ndomains)
      (fix-field k)))
  (when (not (eql y nil))
...
;;Devia modularizar isso
(defun smart-apply1 (constraints on-brutus)
  (let ((adjusted-constraints '()))
    (dolist (constraint constraints)
      (let ((nsols (calculate-nsols on-brutus)))
	(when (= nsols 0) ;There are no solutions down this path
	    (return))
	;;makes a lot of diference in the extra ones:
	;;if the rules determine a solution before parsing all of then.
	;;Breaks inside the backtrack
	...
	((and (equal (car constraint) '=)
	 ...
	 (let* ((att-1 (cadr constraint))
	   ...
	   ;;Se ele só for 1 em um não precisa readicionar a regra
	   (let ((flag 0))
	     (let ((count 0))
	       (dotimes (x nhouses)
	     ...
	     (when (= flag 0)
	       (push constraint adjusted-constraints)))
	   (fix-field att-1-domain)
	   (fix-field att-2-domain)
	   ))
	;;Resolver as regras do tipo (= (+ coisa n) coisa 2)
	((and (equal (car constraint) '=)
	      (listp (cadr constraint))
	      (equal '+ (caadr constraint)))
	 (let* ((att-1 (cadadr constraint))
	   ...
	   (let ((flag 0))
	     (let ((count 0))
	       ...
	       ;;Nem preciso olhar as ultimas n casas
	       (dotimes (x (- nhouses n))
	     ...
	     (let ((count 0))
	       ...
	       ;;Nao preciso olhar as primeiras n casas
	       (dotimes (x nhouses)
	 ...
	 (let* ((att-1 (cadr (cadadr constraint)))
	   ...
	   (let ((2n (* 2 n)))
	     (dotimes (x (- nhouses 2n))
	       (when (and (= 0 (aref field x att-1-domain att-1-item))
			  (= 0 (aref field (+ x 2n) att-1-domain att-1-item)))
		 (setf (aref field (+ x n) att-2-domain att-2-item) 0))
	       (when (and (= 0 (aref field x att-2-domain att-2-item))
			  (= 0 (aref field (+ x 2n) att-2-domain att-2-item)))
		 (setf (aref field (+ x n) att-1-domain att-1-item) 0))))
	...
	((and (equal (car constraint) 'OR))
	 (let* ((att-0 (cadadr constraint))
	   ...
	   (let ((count 0))
	   (dotimes (x nhouses)
	     (unless (or (= (aref field x att-1-domain att-1-item) 1)
			 (= (aref field x att-2-domain att-2-item) 1))
...
(defun choose-x-y (x y)
  ...
  (let ((out 0))
	...
	(when (and (not (= 1 (aref field_aux myx myy)))
		   (or (> myx x) (and (= myx x) (> myy y))))
	  (setf x myx)
	  (setf y myy)
	  (setf out 1)
	  (return)))
...
(defun smart-apply-repeat (constraints onbrutus)
  (let ((nsols (calculate-nsols onbrutus)))
    (setf constraints (smart-apply1 constraints onbrutus))
    (let ((nsols1 (calculate-nsols onbrutus)))
      (unless (or (= nsols1 1) (= nsols nsols1))
	(setf constraints (smart-apply-repeat constraints onbrutus))
	)))
  constraints)

(defun brutus (constraints x y)
  "Choose one position in array to brute-force, the position should be after x y"
  ...
  (let ((old-field (copy-field field)) (flag nil))
	...
	(when (= 1 (aref field x y z))
	  ...
	  (smart-apply-repeat constraints t)
...
(defun solver (path)
  (with-open-file (f path)
	...
	(compile 'calculate-nsols) ;Good, from 0.3 to 0.2
      ...
      (setf constraints (smart-apply-repeat constraints nil))
      ;;E aqui começa o brute
      (let ((cnt 0))
	(dotimes (z nhouses)
	  (setf cnt (+ cnt (aref field 0 0 z))))
      (unless (= (calculate-nsols nil) 1) ;;Here it should be as I use the field_aux
	(if (= 1 cnt)
...
(defun generate-solution-from-field ()
  (let ((output '()))
	 ...
	 (dotimes (x nhouses)
	   (dotimes (z nhouses)
	     (when (= 1 (aref field x y z))
	       (let* ((house-number x) (item-number z)
		      (item-name (cdr (assoc (cons y item-number) domain-item-inverse-hash :test #'equal))))
		 (push (list item-name (1+ house-number)) output)
		 )
  )))) (nreverse output) ))
...
(defun calculate-nsols (&optional onbrutus)
  (let ((acc 1) (flag 1))
